 ...
 */
//Prüfung Aktuelle Plugin Version
define('B2SVERSION', '2.1.0');
...
function b2sActivate() {
    ...
    $wpdb->query("ALTER TABLE `b2s_filter` ADD `sched_network_date` DATETIME NOT NULL DEFAULT '0000-00-00 00:00:00' AFTER `post_id`;");
...
function b2sAdminMenu() {
    ...
    if (isset($_GET['b2sConnect']) && !empty($_GET['b2sConnect'])) {
        switch ($_GET['b2sConnect']) {
            ...
            case 'b2sPlugin':
                ...
                break;
            case 'editB2S':
                require_once 'helper/B2S/Edit.php';
                break;
            default :
    ...
    //VersionCheck
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://developer.blog2social.com/wp/v2/versionCheck.txt');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    $currentVersion = explode('#', curl_exec($ch));
    $b2sUpdate = ($currentVersion[0] != B2SVERSION) ? 1 : 0;  //1=InfoUpdate
...
function b2sInit() {
    ...
    wp_register_style('B2SDATECSS', plugin_dir_url(__FILE__) . 'assets/lib/datetimepicker/css/DateTimePicker.css');
    ...
    wp_register_script('B2SMODAL', plugin_dir_url(__FILE__) . 'assets/lib/modal/js/jquery.leanModal.min.js');
    wp_register_script('B2SDATEJS', plugin_dir_url(__FILE__) . 'assets/lib/datetimepicker/js/DateTimePicker.js');
...
//PageFunktion
function b2sContent() {
    wp_enqueue_script('B2SMODAL');
    wp_enqueue_script('B2SJS');
    wp_enqueue_script('B2SCOUNTDOWNJS');
    wp_enqueue_style('B2SDATECSS');
    wp_enqueue_script('B2SDATEJS');
    $language = (!in_array(get_locale(), unserialize(B2SPLUGINLANGUAGE))) ? 'en_US' : get_locale();
    $lang = parse_ini_file(plugin_dir_path(__FILE__) . 'languages/' . $language . '.ini');
