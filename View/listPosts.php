...
if (!current_user_can('unfiltered_html')) {
    $addUser = $wpdb->prepare(' AND `post_author` = %d', $currentUserID);
}
...
$sqlPostsPage = "SELECT `$wpdb->posts`.`ID`, `post_author`, `post_date`, `post_title`, `sent`.`prgsent`, `sent`.`twittersent`, `sent`.`facebooksent` 
		...
		WHERE `post_status` = 'publish' AND `post_type` = 'post' $addUser $addWhere $addSearch
		ORDER BY `ID` DESC
    ...
    <div>		
        <div class="pull-right btn-group">
            ...
            <ul class="dropdown-menu" role="menu">
                ...
                <li>
                    <a href="?page=blog2social&filterSet=storify<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=blog2social&filterSet=storify<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['storify']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_STORIFY_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=blog2social&filterSet=pinterest<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=blog2social&filterSet=pinterest<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['pinterest']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_PINTEREST_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=blog2social&filterSet=flickr<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=blog2social&filterSet=flickr<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['flickr']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_FLICKR_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=blog2social&filterSet=xing<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=blog2social&filterSet=xing<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['xing']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_XING_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=blog2social&filterSet=diigo<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=blog2social&filterSet=diigo<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['diigo']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_DIIGO_SENT_MSG']; ?>
                    </a>
                </li>
                <li>
                    <a href="?page=blog2social&filterSet=googleplus<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>">
                        <input onclick="window.open('?page=blog2social&filterSet=googleplus<?php echo!empty($_GET['prgSearch']) ? '&prgSearch=' . $_GET['prgSearch'] : ''; ?>', '_self');" <?php echo ($filters['googleplus']) ? 'checked="checked"' : '' ?> type="checkbox"> <?php echo $text['TO_GOOGLEPLUS_SENT_MSG']; ?>
                    </a>
                </li>
            </ul>
        </div>
