...
<?php } else if (!isset($_GET['reportId']) || (int) $_GET['reportId'] == 0) { ?>
    ...
    $sqlReport = "SELECT `post_id`, `last_network_publish_date`, `last_prg_publish_date`, `publishData`
    ...
    if (!empty($reportResult) && isset($reportResult[0]->last_network_publish_date)) {
        ...
        $reportData = (!empty($reportResult[0]->publishData)) ? unserialize(stripslashes($reportResult[0]->publishData)) : '';
    }
    ?>
                ...
                <table class="table noborder">
                    ...
                    foreach($reportData as $r => $report) {
                        ...
                        <tr class="border-bottom">
                            ...
                            <td width="10%"><div class="report-portal"><?php echo $report['type'].' | '.$report['portal_name']; ?></div></td>
                           ...
                           <td width="30%"><div class="report-info-fail">
                             <?php if((int)$report['error'] == 1){
                                 if($report['type'] == 'profil'){
                                      echo $lang['REPORT_FAILED_PROFIL'];
                                 }
                                  if($report['type'] == 'page'){
                                      echo $lang['REPORT_FAILED_PAGE'];
                                 }
                                  if($report['type'] == 'group'){
                                      echo $lang['REPORT_FAILED_GROUP'];
                                 }
                             } ?>
                           </td>
                        </tr>
