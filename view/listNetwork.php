        ...
        <div class="col-md-7 col-xs-12 leftdiv network">
            <div id="portale">
                ...
                <table class="table noborder">
                    ...
                    foreach ($resultPortale->portale as $p => $portal) {
                        if ((int) $portal->portal_aktiv == 1) {
                            $profil_multi = (isset($portal->profil_multi)) ? (int) $portal->profil_multi : 0;
                            ...
                            $accountName = (empty($portal->profil_name) && !isset($portal->profil_name) && $profil_multi == 0) ? '' : $portal->profil_name;
                            ...
                            $accountName = (empty($accountName) && $profil_multi == 1) ? 'Buisness Account*' : $accountName;
                            ...
                            $expired_date = (isset($portal->expired_date) && $portal->expired_date != '0000-00-00') ? '<span class="network-auth-info"><img class="warning-image" src="' . plugins_url('/assets/images/warning.png', dirname(__FILE__)) . '"> ' . $lang['NETWORK_EXPIRED_DATE_TITLE'] . ' ' . B2SUtil::getCustomDateFormatShort($portal->expired_date . ' 00:00:00', substr(B2SLANGUAGE, 0, 2)) . '</span>' : '';
                            $mandant_id = (isset($portal->mandant_id) || (int) $aktivMandant > 0) ? ((isset($portal->mandant_id)) ? $portal->mandant_id : $aktivMandant ) : 0;
                            $b2sAuthUrl = 'https://developer.blog2social.com/wp/v2/network/auth.php?b2s_token=' . B2STOKEN . '&portal_id=' . $portal->id . '&transfer=' . $portal->transfer . '&sprache=' . substr(B2SLANGUAGE, 0, 2) . '&mandant_id=' . $mandant_id;
                            ?>
                            <tr>
                                <td>
                                    <img src="<?php echo plugins_url('assets/images/portale/' . $portal->id . '_flat.png', dirname(__FILE__)); ?>"></td>
                                <td>
                                    <div class="portal-name"><?php echo $lang['NETWORK_PROFIL_NAME'] ?><?php echo ' | ' . $portal->name; ?> <span class="account-name account-name-<?php echo $portal->id; ?>" style="display:<?php echo (empty($accountName)) ? 'none' : 'block'; ?>;" >- <?php echo $accountName; ?></span> <?php echo $expired_date; ?></div>
                                </td>
                                <td>
                                    <div class="network-auth-button">
                                        <a href="#" id="portal-<?php echo $portal->id; ?>" class="btn btn-<?php echo $connectProfile[0]; ?> userLogin"
                                           onclick="wop('<?php echo $b2sAuthUrl . '&choose=profile'; ?>', 'Blog2Social Network', 'netzwerk',<?php echo $mandant_id; ?>);
                                                   return false;"><?php echo $connectProfile[1]; ?></a>
                ...
                <?php if (isset($portal->page) && (int) $portal->page == 1) { ?>
                                <tr>
                                    <td>
                                        <img src="<?php echo plugins_url('assets/images/portale/' . $portal->id . '_flat.png', dirname(__FILE__)); ?>">
                                    </td>
                                    <td>
                                        <div class="portal-name"><?php echo $lang['NETWORK_PAGE_NAME']; ?><?php echo ' | ' . $portal->name; ?> <span class="portal-name"> <a id class="info-network" name="info-network" href="#info-network" rel="page-info"><img class="network-info" src="<?php echo plugins_url('assets/images/b2s_info_small.png', dirname(__FILE__)); ?>"></a> <span class="page-name page-name-<?php echo $portal->id; ?>" style="display:<?php echo (empty($pageId)) ? 'none' : 'block'; ?>;" >- <?php echo $pageName . $pageId; ?></span> </span></div>
                                    </td>
                                    <td>
                                        <div class="network-auth-button">
                                            <a href="#" id="portal-<?php echo $portal->id; ?>" class="btn btn-<?php echo $connectPage[0]; ?> userLogin"
                                               onclick="wop('<?php echo $b2sAuthUrl . '&choose=page'; ?>', 'Blog2Social Network', 'netzwerk',<?php echo $mandant_id; ?>);
                                                       return false;"><?php echo $connectPage[1]; ?></a>
                                        </div>
                                    </td>
                                </tr>    
                <?php } ?>
                ...
                <?php if (isset($portal->group) && (int) $portal->group == 1) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="portal-name"><?php echo $lang['NETWORK_GROUP_NAME']; ?><?php echo ' | ' . $portal->name; ?> <span class="portal-name"> <a id class="info-network" name="info-network" href="#info-network" rel="group-info"><img class="network-info" rel="group-info" src="<?php echo plugins_url('assets/images/b2s_info_small.png', dirname(__FILE__)); ?>"></a> <span class="group-name group-name-<?php echo $portal->id; ?>" style="display:<?php echo (empty($groupId)) ? 'none' : 'block'; ?>;" >- <?php echo $groupName . $groupId; ?></span> </span></div>
                                    </td>
                                    <td>  
                                        <div class="network-auth-button">   
                                            <a href="#" id="portal-<?php echo $portal->id; ?>" class="btn btn-<?php echo $connectGroup[0]; ?> userLogin"
                                               onclick="wop('<?php echo $b2sAuthUrl . '&choose=group'; ?>', 'Blog2Social Network', 'netzwerk',<?php echo $mandant_id; ?>);
                                                       return false;"><?php echo $connectGroup[1]; ?></a>
                                        </div>
                                    </td>
                                </tr>
                            <?php } ?>
        ...
        <?php if (!empty($resultMandanten) && is_object($resultMandanten) && isset($resultMandanten->result) && (int) $resultMandanten->result == 1 && is_array($resultMandanten->mandant) && !empty($resultMandanten->mandant)) { ?>
                    ...
                    foreach ($resultMandanten->mandant as $p => $mandant) {
                        ...
                        <div class="list-group-item <?php echo $classActive; ?>">
                            <h4 class="list-group-item-heading">
                                <div class="pull-left"><img src="<?php echo plugins_url('assets/images/b2s_mandant.png', dirname(__FILE__)); ?>" class="img-icon hidden-sm hidden-xs hidden-md"></div>
                                <div id="mandant-name-<?php echo $mandant->id; ?>" class="mandant-list-name pull-left"><?php echo (strlen(stripslashes($mandant->name)) >= 30) ? substr(stripslashes($mandant->name), 0, 26) . '...' : stripslashes($mandant->name); ?></div>
                                <div class="pull-right">
