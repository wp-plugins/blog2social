...
if (isset($postData['checkXing']) && $postData['checkXing'] == 'on') {
    $addUrl = $postData['urlForXing'] == '' ? '' : ' ' . $postData['urlForXing'];
    $post .= '&inputs%5B8%5D%5Bmessage%5D=' . urlencode($postData['prg_xing'] . $addUrl);
}

if (isset($postData['checkDiigo']) && $postData['checkDiigo'] == 'on') {
    $addToDiigoPost = '&inputs%5B9%5D%5Btags%5D=';
    foreach ($postData['prg_diigo_tags'] as $tag) {
        $addToDiigoPost .= $tag . ',';
    }

    $post .= '&inputs%5B9%5D%5Btitel%5D=' . urlencode($postData['postTitle']) .
            '&inputs%5B9%5D%5Blink%5D=' . urlencode($postData['shortUrl']) .
            '&inputs%5B9%5D%5Bmessage%5D=' . urlencode($postData['prg_diigo']) .
            $addToDiigoPost;
}

if (isset($postData['checkGoogleplus']) && $postData['checkGoogleplus'] == 'on') {
    $post .= '&inputs%5B10%5D%5Bpicture%5D=' . urlencode($postData['image']) .
            '&inputs%5B10%5D%5Blink%5D=' . urlencode($postData['shortUrl']) .
            '&inputs%5B10%5D%5Bmessage%5D=' . urlencode($postData['prg_googleplus']);
}
