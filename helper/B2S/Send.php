    ...
    if (isset($_POST['mandant_id']) && !empty($_POST['token']) && isset($_POST['token']) && (int) $_POST['blog_user_id'] > 0 && (int) $_POST['post_id'] > 0) {
        ...
        if (is_object($result) && !empty($result) && isset($result->result)) {
            if ($result->result == 1) {
                global $wpdb;
                //Filter
                $sqlCheckPost = $wpdb->prepare("SELECT `id` FROM `b2s_filter` WHERE `post_id` = %s", $_POST['post_id']);
                $postEntry = $wpdb->get_var($sqlCheckPost);
                $currentDateTime = current_time('mysql');
                if ($postEntry == NULL || (int)$postEntry == 0) {
                    $wpdb->insert('b2s_filter', array('last_network_publish_date' => $currentDateTime, 'publishData' => $result->data, 'post_id' => (int) $_POST['post_id'], 'blog_user_id' => (int) $_POST['blog_user_id']));
                    $postEntry = $wpdb->insert_id;
                } else {
                    $wpdb->update('b2s_filter', array('last_network_publish_date' => $currentDateTime, 'publishData' => $result->data), array('post_id' => (int) $_POST['post_id'], 'blog_user_id' => (int) $_POST['blog_user_id']));
                }
                if ((int) $postEntry > 0) {
