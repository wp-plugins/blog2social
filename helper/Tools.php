...
class B2STools {
    public static function getToken($blog_user_id = 0,$firstName='',$lastName='',$email='',$blog_url = '') {
        ...
        $url = 'https://developer.blog2social.com/wp/v1/post.php';
    ...
    public static function getPortale($token = '', $portal_id = 0, $mandant_id = 0) {
     ...
     public static function getInfo($token = '',$type='') {
        if (empty($token)) {
            return false;
        }
        $url = 'https://developer.blog2social.com/wp/v1/post.php';
        $post = array('action' => 'getInfo', 'token' => $token,'type' => $type);
        return self::curlPost($url, $post);
    }
    ...
    public static function getPRGCategory() {
        $url = 'http://developer.pr-gateway.de/wp/v1/get.php?action=getCategory';
        return self::curlGet($url);
    }
    public static function getPRGCountry() {
        $url = 'http://developer.pr-gateway.de/wp/v1/get.php?action=getCountry';
        return self::curlGet($url);
    }
    public static function createMessagePRG($datas = array()) {
        $url = 'http://developer.pr-gateway.de/wp/v1/post.php';
        return self::curlPost($url, $datas);
    }
    public static function sentToNetwork($datas = array()) {
        $url = 'https://developer.blog2social.com/wp/v1/post.php';
        return self::curlPost($url, array('action' => 'sentToNetwork', 'data' => serialize($datas)));
    }
    ...
    public static function loginPRG($username, $password) {
        $loginUrl = 'http://developer.pr-gateway.de/wp/v1/auth.php';
